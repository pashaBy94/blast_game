...
function createTail(t, l, row, col, color, isBomb = false) {
    ...
    if (isBomb) {
        coin.style.boxShadow = `2px -2px 5px black, 5px -5px 5px black, 8px -8px 5px black,inset 0 0 10px 8px rgba(161, 161, 161, 0.592)`;
    } else {
        coin.style.boxShadow = `2px -2px 5px ${color}, 5px -5px 5px ${color}, 8px -8px 5px ${color},inset 0 0 10px 8px rgba(161, 161, 161, 0.592)`;
    }
    tail.append(coin);
    ...
    if (isBomb)
        bi.innerHTML = `<img src="imag/pow.png" />`;
    else
